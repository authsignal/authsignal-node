...
import {SmsChallengeRequest, OtpChallengeResponse, OtpVerifyRequest, OtpVerifyResponse} from "../types";
import {AuthsignalFlowsClient} from "./client";

export class AuthsignalFlowsSms extends AuthsignalFlowsClient {
  async challenge(request: SmsChallengeRequest): Promise<OtpChallengeResponse> {
    const {challengeToken, phoneNumber} = request;

    const url = `${this.apiUrl}/client/flows/challenge/sms`;
  ...
  async verify(request: OtpVerifyRequest): Promise<OtpVerifyResponse> {
    ...
    const url = `${this.apiUrl}/client/flows/verify/sms`;
